...
class message_bus {
  public:
	template<typename Function> void register_handler(std::string const & name, const Function& f) {
	...
	template <typename T, typename ... Args> T call(const std::string& name, Args&& ... args) {
	...
	template <typename ... Args> void call_void(const std::string& name, Args&& ... args) {
	  auto it = invokers_.find(name);
	  if (it == invokers_.end())
	  return;
