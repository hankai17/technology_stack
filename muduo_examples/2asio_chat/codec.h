...
class LengthHeaderCodec : muduo::noncopyable {
 public:
  ...
  void send(muduo::net::TcpConnection* conn, const muduo::StringPiece& message) { //used for client
  ...
  void send1(muduo::net::TcpConnection* conn, const muduo::StringPiece& message) { //used for client
    muduo::net::Buffer buf;
    buf.append(message.data(), message.size());
    int32_t len = static_cast<int32_t>(message.size());
    int32_t be32 = muduo::net::sockets::hostToNetwork32(len);
    buf.prepend(&be32, sizeof be32);
    //conn->send(&buf);
    std::string tmp_buf = buf.retrieveAllAsString();
    for(int i = 0; i < tmp_buf.size(); i++) {
      muduo::net::Buffer buf;
      conn->send((const void*)tmp_buf[i], 1);
      sleep(1);
    }
  }
