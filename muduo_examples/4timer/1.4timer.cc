...
class Printer : muduo::noncopyable {
 public:
  ...
  void print1() {
    ...
    if (shouldQuit) { // out of lock // printf("loop1_->quit()\n");
      loop1_->quit();
    } else {
      printf("Timer 1: %d\n", count); // cout is not thread safe //std::cout << "Timer 1: " << count << "\n";
      loop1_->runAfter(1, std::bind(&Printer::print1, this));
    }
  }
  ...
  void print2() {
    ...
    if (shouldQuit) { // out of lock // printf("loop2_->quit()\n");
      loop2_->quit();
    } else { // cout is not thread safe //std::cout << "Timer 2: " << count << "\n";
      printf("Timer 2: %d\n", count);
      loop2_->runAfter(1, std::bind(&Printer::print2, this));
    }
  }
