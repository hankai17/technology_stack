...
void serverConnectionCallback(const TcpConnectionPtr& conn) {
  ...
  if (conn->connected()) {
    conn->setTcpNoDelay(true); //服务端 链接到来设nodelay
  } else {}
}

void serverMessageCallback(const TcpConnectionPtr& conn, Buffer* buffer, muduo::Timestamp receiveTime) {
  int64_t message[2];
  while (buffer->readableBytes() >= frameLen) { //连发两个
...
void clientConnectionCallback(const TcpConnectionPtr& conn) { //只调一次
  LOG_TRACE << conn->localAddress().toIpPort() << " -> "
        << conn->peerAddress().toIpPort() << " is "
        << (conn->connected() ? "UP" : "DOWN");
  if (conn->connected()) { //链接成功
...
void clientMessageCallback(const TcpConnectionPtr&, Buffer* buffer, muduo::Timestamp receiveTime) {
  int64_t message[2];
...
void runClient(const char* ip, uint16_t port) { //  EventLoop loop;
  EventLoop loop;
