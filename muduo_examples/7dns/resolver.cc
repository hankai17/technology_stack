#include "examples/cdns/resolver.hh"
...
namespace {
  double getSeconds(struct timeval* tv) {
    if (tv) return double(tv->tv_sec) + double(tv->tv_usec)/1000000.0;
    else return -1.0;
  }
  
  const char* getSocketType(int type) {
    if (type == SOCK_DGRAM) return "UDP";
    else if (type == SOCK_STREAM) return "TCP";
    else return "Unknown";
  }
...
Resolver::Resolver(EventLoop* loop, Option opt)
  ...
  options.flags |= ARES_FLAG_IGNTC; //只开udp
  ...
  int status = ares_init_options(&ctx_, &options, optmask); //传出ares_channel ctx
  if (status != ARES_SUCCESS) {
    assert(0);
  }
  ares_set_socket_callback(ctx_, &Resolver::ares_sock_create_callback, this); //设置open_udp/tcp的回调
}
...
bool Resolver::resolve(StringArg hostname, const Callback& cb) { //1上层直接调
...
void Resolver::onRead(int sockfd, Timestamp t) { //3select回调
...
void Resolver::onQueryResult(int status, struct hostent* result, const Callback& callback) { //4
...
void Resolver::onSockCreate(int sockfd, int type) { //2 ares_connect
  loop_->assertInLoopThread();
  assert(channels_.find(sockfd) == channels_.end());
  Channel* channel = new Channel(loop_, sockfd); //ares发包前拿到它的fd 初始化成muduo的channel
...
void Resolver::ares_host_callback(void* data, int status, int timeouts, struct hostent* hostent) { //4ares解析dns成功 回调上层
  QueryData* query = static_cast<QueryData*>(data);
  query->owner->onQueryResult(status, hostent, query->callback);
  delete query;
}

int Resolver::ares_sock_create_callback(int sockfd, int type, void* data) { //2
