...
namespace sylar {
    namespace quic {
        ...
        void BandwidthSampler::onPacketSent(uint64_t sent_time, uint64_t pn, uint64_t bytes, 
                uint64_t bytes_inflight, bool has_retransmittable_data) { // 记录每个包的信息
            ...
            if (bytes_inflight == 0) { // 若没有在途包: 第一次发包 或 数据缺口后第一次发包
    ...
    // lap 三剑客: 发包时总字节数 发包时间 收ack时间
            m_total_bytes_sent_at_last_acked_packet = sent_packet->totalBytesSent; // 收到的这个最新的ack对应的发包时刻 已发送的总字节数
            m_last_acked_packet_sent_time = sent_packet->sentTime; // 收到的这个最新的ack对应的发包时刻
            m_last_acked_packet_ack_time = ack_time; // 收到ack的时刻
            ...
            uint64_t sent_rate = ~0ull;                                         //                          --------------->t
            if (sent_packet->sentTime < sent_packet->lastAckedPacketSentTime) { // 收到的这个最新ack对应的发包时刻 < 收到的这个最新ack对应的发包时刻 时 收到的最新ack对应的发包时刻
            ...
            uint64_t ack_rate = BandwidthFromDelta(m_total_bytes_acked - sent_packet->totalBytesAckedAtTheLastAckedPacket,
                    ack_time - sent_packet->lastAckedPacketAckTime); // 收到ack时累积的总字节数 - 这个ack对应的发包时刻时 ack的总字节数
                                                                     // 收到ack的时间 - 这个ack对应的发包时刻 收到最新ack时的时间
