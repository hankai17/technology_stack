...
long get_current_time() {
    ...
    return ((long)l_now.tv_sec)*1000*1000+(long)l_now.tv_usec;
}
...
int main (int argc, char *argv[]) {
    ...
    while((c = getopt(argc, argv, "qt:u:v:n:i:")) != -1) {
        switch(c) {
            case 'q':
                verbose = 0;
                 break;
            ...
            case 'n':
                cptr = NULL;
                num = strtol(optarg, &cptr, 10);
                break;
            case 'i':
                cptr = NULL;
                interval = strtol(optarg, &cptr, 10);
                break;
            default:
                usage(argv[0]);
                break;
...
ok:
        ...
        close(sockfd);
        if (verbose)
            fprintf(stdout, "%s port %s open ttl: %ld(us)\n", argv[optind], argv[optind+1],  end_time - begin_time);
    }
    return 0;
}
...
void usage(char *prog) {
    fprintf(stderr, "error: Usage: %s [-q] [-t timeout_sec] [-u timeout_usec] [-v4|6] [-n request times] [-i each request interval(s)] <host> <port>\n", prog);
        exit(-1);
}

//https://blog.csdn.net/wolf_break/article/details/38559599
//https://github.com/MushrooM93/tcping
