...
class LRUCacheDict(object):
	...
	for k in self.__values.keys():
	    print k, self.__values[k]

    def traversal1(self):
    	for k in self.__access_times.iterkeys():
	    print k, self.__values[k]

    def traversal_weight_gt(self, weight):
	arr = []
    	for k in self.__access_times.iterkeys():
	    if self.__values[k] > weight:
		arr.append(k)
	return arr
    ...
    @_lock_decorator
    def __setitem__(self, key, value): #value must == 1
        t = int(time.time())
	weight = 0
	if self.has_key(key):
	    weight = self.__values[key]
	weight = weight + 1

        self.__delete__(key)
        self.__values[key] = weight
...
'''
'''
d = LRUCacheDict(max_size=5, expiration=30)
d['domain1'] = 1
d['domain2'] = 1
d['domain3'] = 1
d['domain3'] = 1
