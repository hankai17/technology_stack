...
static int sock_cb(CURL *e, curl_socket_t s/*fd*/, int what/*描述的是s*/, void *cbp, void* sockp/*我的私有变量*/) { //这个sockp是什么
...
int main(int argc _Unused, char** argv _Unused) {
  ...
  ev.events = EPOLLIN;
  ev.data.fd = g.tfd;
  epoll_ctl(g.epfd, EPOLL_CTL_ADD, g.tfd, &ev); //把timefd挂树上

  init_fifo(&g); //把fifo读端挂树上
  ...
  while(!g_should_exit_) {
    ...
    err = epoll_wait(g.epfd, events, sizeof(events)/sizeof(struct epoll_event), 10000);
    if(err == -1) {
      if(errno == EINTR) {
...
//还是libevent封装的好看一点
//为什么这个模型用sock_cb  而上一个模型不用 用sock有什么好处?
