...
static int ng_encode_arp_pkt(uint8_t *msg, uint8_t *dst_mac, uint32_t sip, uint32_t dip) {
    ...
    struct rte_arp_hdr *arp = (struct rte_arp_hdr *)(eth + 1);          // 2 arp 
...
#if ENABLE_ICMP
static uint16_t ng_checksum(uint16_t *addr, int count) {                                    // RFC 1071
    register long sum = 0;
    while (count > 1) {
...
static int ng_encode_icmp_pkt(uint8_t *msg, uint8_t *dst_mac,
    ...
    struct rte_ether_hdr *eth = (struct rte_ether_hdr *)msg;                                // 1 ether
    rte_memcpy(eth->s_addr.addr_bytes, gSrcMac, RTE_ETHER_ADDR_LEN);
    rte_memcpy(eth->d_addr.addr_bytes, dst_mac, RTE_ETHER_ADDR_LEN);
    eth->ether_type = htons(RTE_ETHER_TYPE_IPV4);
    struct rte_ipv4_hdr *ip = (struct rte_ipv4_hdr *)(msg + sizeof(struct rte_ether_hdr)); // 2 ip
    ip->version_ihl = 0x45;
    ip->type_of_service = 0;                                                                // 普通优先级
    ip->total_length = htons(sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_icmp_hdr));
    ip->packet_id = 0;                                                                      // IP 分包时用的，用来标记 “哪几个分片属于同一个包”  不分片，直接填 0 
    ip->fragment_offset = 0;
    ip->time_to_live = 64;
    ...
    ip->dst_addr = dip;
    ip->hdr_checksum = rte_ipv4_cksum(ip);
    struct rte_icmp_hdr *icmp = (struct rte_icmp_hdr *)(msg +                               // 3 icmp 
            sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr));
    icmp->icmp_type = RTE_IP_ICMP_ECHO_REPLY;
    icmp->icmp_code = 0;                                                                    // 回显请求
    icmp->icmp_ident = id;                                                                  // 标记是 “哪个进程 / 程序” 发的 ping
    icmp->icmp_seq_nb = seqnb;                                                              // 标记这是第几个 ping 包
    icmp->icmp_cksum = 0;                                                                   // 必须先清 0，再计算！因为校验和计算时不能包含自己
