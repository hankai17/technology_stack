...
static struct rte_mbuf *ng_send_arp(struct rte_mempool *mbuf_pool, uint16_t opcode,
...
arp_request_timer_cb(__attribute__((unused)) struct rte_timer *tim, void *arg) {
	...
	for (i = 1; i <= 254; i++) {
		...
		if (dstmac == NULL) {
			...
			arpbuf = ng_send_arp(mbuf_pool, RTE_ARP_OP_REQUEST, dstmac, gLocalIp, dstip);   // 续命
		}
		//rte_eth_tx_burst(gDpdkPortId, 0, &arpbuf, 1);
		//rte_pktmbuf_free(arpbuf);
...
struct ng_tcp_stream {                  // tcb control block
	...
	uint32_t snd_nxt;                   // 本次发送的包的seqnum
	uint32_t rcv_nxt;                   // 
...
static struct ng_tcp_stream * ng_tcp_stream_search(uint32_t sip, uint32_t dip,
...
static int ng_tcp_handle_listen(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr, struct rte_ipv4_hdr *iphdr) {
	if (tcphdr->tcp_flags & RTE_TCP_SYN_FLAG)  {
		if (stream->status == NG_TCP_STATUS_LISTEN) {
            ...
            syn->rcv_nxt = fragment->acknum;                                                    // realiable1
...
static int ng_tcp_handle_syn_rcvd(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr) {
	if (tcphdr->tcp_flags & RTE_TCP_ACK_FLAG) {
		if (stream->status == NG_TCP_STATUS_SYN_RCVD) {
			...
			struct ng_tcp_stream *listener = ng_tcp_stream_search(0, 0, 0, stream->dport);      // hakai2 accept
			if (listener == NULL) {
...
static int ng_tcp_enqueue_recvbuffer(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr, int tcplen) {
	...
	pthread_cond_signal(&stream->cond);                                     // hankai4 接收到数据封装成frag排入队列 通知上层读取
...
static int ng_tcp_send_ackpkt(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr) {
	...
	ackfrag->seqnum = stream->snd_nxt;                                      // realiable1
...
static int ng_tcp_handle_established(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr, int tcplen) {
	...
	if (tcphdr->tcp_flags & RTE_TCP_PSH_FLAG) {
#if 0
		struct ng_tcp_fragment *rfragment = rte_malloc("ng_tcp_fragment",   // 构造上层接收报文 仅构造tcp层
                sizeof(struct ng_tcp_fragment), 0);
		if (rfragment == NULL) return -1;
		memset(rfragment, 0, sizeof(struct ng_tcp_fragment));
		rfragment->dport = ntohs(tcphdr->dst_port);
		rfragment->sport = ntohs(tcphdr->src_port);
...
#endif
#if 0
		struct ng_tcp_fragment *ackfrag = rte_malloc("ng_tcp_fragment",     // 构造ack 仅构造tcp层
                sizeof(struct ng_tcp_fragment), 0);
		if (ackfrag == NULL) return -1;
		memset(ackfrag, 0, sizeof(struct ng_tcp_fragment));
		stream->rcv_nxt = stream->rcv_nxt + payloadlen;                     // 更新seq/ack: seq更新为接收的ack
		...
		uint8_t hdrlen = tcphdr->data_off >> 4;
		int payloadlen = tcplen - hdrlen * 4;
		stream->rcv_nxt = stream->rcv_nxt + payloadlen;                     // realiable1
		stream->snd_nxt = ntohl(tcphdr->recv_ack);
		ng_tcp_send_ackpkt(stream, tcphdr);
#endif
#if 0
		struct ng_tcp_fragment *echofrag = rte_malloc("ng_tcp_fragment",    // 构造echo报文 仅构造tcp层
                sizeof(struct ng_tcp_fragment), 0);
...
#endif
		stream->rcv_nxt = stream->rcv_nxt + 1;                              // realiable1
		stream->snd_nxt = ntohl(tcphdr->recv_ack);
		ng_tcp_send_ackpkt(stream, tcphdr);
	}
	return 0;
...
static int udp_out(struct rte_mempool *mbuf_pool) {
	...
	for (host = lhost; host != NULL; host = host->next) {
		...
		if (dstmac == NULL) {
			...
			struct inout_ring *ring = ringInstance();
			rte_ring_mp_enqueue_burst(ring->out, (void **)&arpbuf, 1, NULL);
			rte_ring_mp_enqueue(host->sndbuf, ol);  // try again 但是没有阈值?
		} else {
...
static int nbind(int sockfd, const struct sockaddr *addr,
	...
	struct localhost *hostinfo =  get_hostinfo_fromfd(sockfd);
...
static ssize_t nrecvfrom(int sockfd, void *buf, size_t len, __attribute__((unused))  int flags,
	...
	if (len < ol->length) {
		...
		rte_free(ol->data);
		ol->data = ptr;
		rte_ring_mp_enqueue(host->rcvbuf, ol);  // 不再通知一下? 不 反正是周期执行的 所以实时性会差一点
		return len;
	} else {
...
static int udp_server_entry(__attribute__((unused))  void *arg) {
	...
	localaddr.sin_addr.s_addr = inet_addr("192.168.0.108"); // 0.0.0.0
...
int main(int argc, char *argv[]) {
	...
	uint64_t hz = rte_get_timer_hz();       // hz: 多少个时钟周期后触发 1s // PERIODICAL: 永久重复 
	unsigned lcore_id = rte_lcore_id();     // 在哪个核上运行回调
...
#if ENABLE_MULTHREAD
    main_lcore = rte_get_next_lcore(main_lcore, 1, 0);
	rte_eal_remote_launch(pkt_process, mbuf_pool, main_lcore);
#endif
#if ENABLE_UDP_APP
    main_lcore = rte_get_next_lcore(main_lcore, 1, 0);
	if (rte_eal_remote_launch(udp_server_entry, mbuf_pool, main_lcore) < 0) {
        printf("udp_server failed\n");
    }
#endif
#if ENABLE_TCP_APP
	main_lcore = rte_get_next_lcore(main_lcore, 1, 0);
	if (rte_eal_remote_launch(tcp_server_entry, mbuf_pool, main_lcore) < 0) {
        printf("tcp_server failed\n");
    }
#endif
	while (1) {
		struct rte_mbuf *rx[BURST_SIZE];                                            // rx
		unsigned num_recvd = rte_eth_rx_burst(gDpdkPortId, 0, rx, BURST_SIZE);
		if (num_recvd > BURST_SIZE) {
			rte_exit(EXIT_FAILURE, "Error receiving from eth\n");
		} else if (num_recvd > 0) {
			rte_ring_sp_enqueue_burst(ring->in, (void**)rx, num_recvd, NULL);       // 单生产者
		}
		struct rte_mbuf *tx[BURST_SIZE];                                            // tx
		unsigned nb_tx = rte_ring_sc_dequeue_burst(ring->out, (void**)tx, BURST_SIZE, NULL);    // 单消费者
		if (nb_tx > 0) {
