...
static void evdns_server_callback(struct evdns_server_request* req, void* data) {
  ...
  for (i = 0; i < req->nquestions; ++i) { //自定义dns报文
    u32 ans = htonl(0xc0a80b0bUL);
    //A记录
    if (req->questions[i]->type == EVDNS_TYPE_A && req->questions[i]->dns_question_class == EVDNS_CLASS_INET) {
      printf(" -- replying for %s (A)\n", req->questions[i]->name);
      r = evdns_server_request_add_a_reply(req, req->questions[i]->name, 1, &ans, 10);
      if (r<0) printf("eeep, didn't work.\n"); //PTR记录
    } else if (req->questions[i]->type == EVDNS_TYPE_PTR && req->questions[i]->dns_question_class == EVDNS_CLASS_INET) {
      printf(" -- replying for %s (PTR)\n", req->questions[i]->name);
      r = evdns_server_request_add_ptr_reply(req, NULL, req->questions[i]->name, "foo.bar.example.com", 10);
      if (r<0) printf("ugh, no luck");
    } else {
...
int main(int c, char **v) 
{
  ...
  while ((opt = getopt(c, v, "xvc:Ts:")) != -1) { //a.out -T
    switch (opt) {
  ...
  if (o.servertest) {
    ...
    evdns_add_server_port_with_base(event_base, sock, 0, evdns_server_callback, NULL); //未决
  }
  if (optind < c) {
    int res;
    if (o.ns)
